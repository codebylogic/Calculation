// ================= GOOGLE SHEET & CLOUDINARY CONFIGURATION =================
...
function refreshUserDataFromCloud(uId) {
    if (!uId) return Promise.resolve();
    return callCloudAPI({ action: "fetch" })
    .then(players => {
        if (players && players.length > 0) {
            const current = players.find(p => p.id === uId);
            if (current) {
                studentRating = parseFloat(current.rating) || 120;
                studentName = current.name || studentName;
                localStorage.setItem('auth_rating', studentRating);
                localStorage.setItem('auth_name', studentName);

                saveAccountToMultiList(uId, studentName, studentRating, current.profilePicUrl || localStorage.getItem(`profile_img_${uId}`));

                if (document.getElementById('home-rating-display')) 
                    document.getElementById('home-rating-display').innerText = studentRating;
                if (document.getElementById('lb-user-rating')) 
                    document.getElementById('lb-user-rating').innerText = studentRating;
                if (document.getElementById('profile-display-rating')) 
                    document.getElementById('profile-display-rating').innerText = studentRating;
            }
        }
    })
    .catch(err => console.error("Cloud Auto-Sync rating error:", err));
}

function saveAccountToMultiList(uId, name, rating, profilePic) {
    let accounts = JSON.parse(localStorage.getItem('saved_accounts')) || [];
    const index = accounts.findIndex(acc => acc.id === uId);
    const accData = { id: uId, name: name, rating: rating, profilePicUrl: profilePic || "" };
    
    if (index > -1) {
        accounts[index] = accData;
    } else {
...
function renderSwitchAccountList() {
    ...
    const currentUserId = localStorage.getItem('auth_userid');
    
    listContainer.innerHTML = '';
    
    if (accounts.length === 0 && currentUserId) {
        saveAccountToMultiList(currentUserId, studentName, studentRating, localStorage.getItem(`profile_img_${currentUserId}`));
    ...
    accounts.forEach(acc => {
        ...
        row.className = "flex items-center justify-between p-2.5 bg-slate-950 rounded-xl border border-slate-800/80";
        
        row.innerHTML = `
            <div class="flex items-center space-x-2 truncate pr-2">
                <span class="text-xs font-bold text-white truncate">${acc.name}</span>
                <span class="text-[10px] font-mono text-slate-400">(${acc.id})</span>
            </div>
            ${isCurrent 
                ? '<span class="text-[10px] text-emerald-400 font-extrabold px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg shrink-0">Active</span>' 
                : `<button onclick="switchUserAccount('${acc.id}')" class="px-3 py-1 bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs rounded-lg transition shadow shrink-0">Switch</button>`
            }
        `;
...
function switchUserAccount(targetUserId) {
    ...
    if (targetAcc) {
        showToast("⏳", `Switching account to ${targetAcc.name}...`);
        
        localStorage.setItem('auth_userid', targetAcc.id);
        localStorage.setItem('auth_name', targetAcc.name);
        localStorage.setItem('auth_rating', targetAcc.rating);
        
        refreshUserDataFromCloud(targetAcc.id).finally(() => {
            location.reload();
        });
    }
}
...
function executeAutoLogin(uId, pass) {
    ...
    .then(resData => {
        if(resData.status === "success") {
            ...
            saveAccountToMultiList(uId, studentName, studentRating, resData.profilePicUrl || "");
...
function handleProfileImageUpload(event) {
    ...
    .then(cloudinaryRes => {
        if (cloudinaryRes.secure_url) {
            ...
            saveAccountToMultiList(playerId, studentName, studentRating, uploadedUrl);
            loadSavedProfileImage();
...
function uploadRatingToCloud() {
    ...
    localStorage.setItem('auth_rating', studentRating);
    saveAccountToMultiList(playerId, studentName, studentRating, localStorage.getItem(`profile_img_${playerId}`));
}
...
function getActivePracticeQuestionTier() {
    if (appSettings.incognitoMode && appSettings.customPracticeTier) {
        return appSettings.customPracticeTier;
    }
    return getEloQuestionTier(studentRating);
}
...
function generateTieredBODMAS(tier) {
    // Balanced Medium difficulty question engine for BODMAS across all tiers:
    // Ensures clean integer answers, varied BODMAS patterns, suitable for mental math.
    const patterns = [
        ...
        () => {
            ...
            if (isSub) {
                const product = b * c;
                const a = randInt(product + 5, product + 40);
        ...
        () => {
            ...
            const isSub = Math.random() > 0.5;
            if (isSub) {
                const c = randInt(5, Math.max(6, product - 5));
                return { questionText: `${a} × ${b} - ${c}`, correctAnswer: product - c };
            } else {
                const c = randInt(10, 45);
                return { questionText: `${a} × ${b} + ${c}`, correctAnswer: product + c };
            }
        },
        // Pattern 3: A + B ÷ C or A - B ÷ C (clean division)
        () => {
            const c = randInt(3, 9);
            ...
            const isSub = Math.random() > 0.5;
            if (isSub) {
                const a = randInt(quotient + 5, quotient + 50);
                return { questionText: `${a} - ${b} ÷ ${c}`, correctAnswer: a - quotient };
            } else {
                const a = randInt(10, 50);
                return { questionText: `${a} + ${b} ÷ ${c}`, correctAnswer: a + quotient };
            }
        },
        // Pattern 4: A ÷ B + C × D or A ÷ B - C × D
        () => {
            const b = randInt(3, 8);
            ...
            const q2 = c * d;
            if (Math.random() > 0.5) {
                return { questionText: `${a} ÷ ${b} + ${c} × ${d}`, correctAnswer: q1 + q2 };
            } else {
                const sum = q1 + q2;
                return { questionText: `${sum} + ${a} ÷ ${b} - ${c} × ${d}`, correctAnswer: sum + q1 - q2 };
            }
        },
        // Pattern 5: (A + B) × C or (A - B) ÷ C with parentheses
        () => {
            if (Math.random() > 0.5) {
                const c = randInt(3, 8);
                const quotient = randInt(3, 10);
                const b = randInt(4, 25);
                const a = (c * quotient) + b;
                return { questionText: `(${a} - ${b}) ÷ ${c}`, correctAnswer: quotient };
            } else {
        ...
        () => {
            if (tier === "gold" || tier === "diamond" || tier === "heroic") {
                ...
                const c = randInt(2, 6);
                const d = randInt(2, 6);
                return { questionText: `${a} × ${b} + ${c} × ${d}`, correctAnswer: (a * b) + (c * d) };
            } else {
                const a = randInt(3, 7);
    ...
    const chosenPattern = patterns[randInt(0, patterns.length - 1)];
...
function generateArithmeticExpression(categoryName) {
    ...
    const activeTier = getActivePracticeQuestionTier();
...
function finishTestRun() {
    ...
    if (!appSettings.incognitoMode) {
        ...
        showToast("🕵️", "Incognito Practice Completed! Rating not modified.");
    }
